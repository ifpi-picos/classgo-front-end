...
export default function SideBar() {
    ...
    return (
        <>
            <nav className={`flex flex-col items-center fixed left-0 float-left overflow-auto w-1/5 h-screen bg-blue-500 text-white shadow-md xs:${isOpen ? "w-4/5 z-50" : "hidden"}`}>
            ...
            <HiMenu className={`fixed hidden z-50 left-8 top-9 text-2xl xs:${isOpen ? "hidden" : "flex"}`} onClick={enableSideBar}/>
        </>
    )
}
